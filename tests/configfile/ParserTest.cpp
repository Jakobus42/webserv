...
TEST_F(ConfigFileParserTest, Configfile_misplaced_semicolon) {
	std::string configFileName =
		"../config/wrong/configfile_misplaced_semicolon";
	ASSERT_EQ(instance.loadConfigFile(configFileName), 1);
	ASSERT_EQ(instance.getIsLoaded(), 0);
}
...
TEST_F(ConfigFileParserTest, ServerName) {
	...
	// correct usage
	{
		std::vector<std::string> args = {"server_name", "localhost",
...
TEST_F(ConfigFileParserTest, ClientMaxBodySize) {
	...
	// correct usage
	{
		...
		int ret =
			instance.testFunction("client_max_body_size", args, lineCount);
	...
	// incorrect usage
	{
		...
		ASSERT_EQ(
			instance.testFunction("client_max_body_size", args, lineCount), 1);
	}
	{
...
TEST_F(ConfigFileParserTest, Index) {
	...
	// correct usage
	{
		...
		ASSERT_EQ(
			instance.getConfigData().servers.back().locations.back().index[0],
			"index.html");
		ASSERT_EQ(
			instance.getConfigData().servers.back().locations.back().index[1],
			"index2.html");
		ASSERT_EQ(ret, 0);
	}
	// incorrect usage
