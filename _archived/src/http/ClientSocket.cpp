...
namespace http {
	...
	int ClientSocket::getFd(void) {
		return m_fd;
	}

	t_sockaddr ClientSocket::getSocketAddress(void) const {
		return m_socketAddress;
	}

	uint32_t ClientSocket::getSocketSize(void) const {
		return m_socketSize;
	}

	bool ClientSocket::isAlive(void) const {
		return m_alive;
	}
