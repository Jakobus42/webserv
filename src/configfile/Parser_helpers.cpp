...
int ConfigFileParser::testFunction(std::string const& key, std::vector<std::string>& args, int& lineCount) {
  int i;
  if (key == "server")
  ...
  if (layer == 0)  // global
  {
    if (qoute_flag == 0) {
      ...
      if (command_id == SERVER_ID) {
        if (server(args, lineCount, layer) == 1) return 1;
      } else {
        LOG("Configuration file (line " << lineCount << "): "
                                        << "Unexpected command found" << std::endl,
            logger::ERROR)
  ...
  } else if (layer == 1)  // server
  {
    if (qoute_flag == 0) {
      if (command_id == LISTEN_ID) {
        if (listen(args, lineCount, layer) == 1) return 1;
      } else if (command_id == SERVER_NAME_ID) {
        if (serverName(args, lineCount, layer) == 1) return 1;
      } else if (command_id == ERROR_PAGE_ID) {
        if (errorPage(args, lineCount, layer) == 1) return 1;
      } else if (command_id == CLIENT_MAX_BODY_SIZE_ID) {
        if (clientMaxBodySize(args, lineCount, layer) == 1) return 1;
      } else {
        LOG("Configuration file (line " << lineCount << "): "
                                        << "Unexpected command found" << std::endl,
            logger::ERROR)
      ...
      if (command_id == LOCATION_ID) {
        if (location(args, lineCount, layer) == 1) return 1;
      } else {
        LOG("Configuration file (line " << lineCount << "): "
                                        << "Unexpected command found" << std::endl,
            logger::ERROR)
  ...
  } else if (layer > 1)  // location
  {
    if (qoute_flag == 0) {
      if (command_id == LIMIT_EXCEPT_ID) {
        if (limitExcept(args, lineCount, layer) == 1) return 1;
      } else if (command_id == RETURN_ID) {
        if (returnKeyword(args, lineCount, layer) == 1) return 1;
      } else if (command_id == ROOT_ID) {
        if (root(args, lineCount, layer) == 1) return 1;
      } else if (command_id == AUTOINDEX_ID) {
        if (autoindex(args, lineCount, layer) == 1) return 1;
      } else if (command_id == INDEX_ID) {
        if (index(args, lineCount, layer) == 1) return 1;
      } else {
        LOG("Configuration file (line " << lineCount << "): "
                                        << "Unexpected command found" << std::endl,
            logger::ERROR)
