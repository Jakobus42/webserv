 ...
 */
int ConfigFileParser::handlePrompt(std::string& line, int layer,
								   int& lineCount) {
	...
	for (unsigned long i = 0; i < m_configData.servers.size(); i++) {
		...
		if (detailed) {
			...
			for (unsigned long j = 0;
				 j < m_configData.servers[i].server_names.size(); j++) {
		...
		printLocations(m_configData.servers[i].locations, 1, detailed,
					   layer_num);
	}
}
...
void ConfigFileParser::printLocations(
	const std::vector<config::t_location>& locations, int layer, int detailed,
	std::vector<int> layer_num) {
	std::string c = "";
	...
	for (unsigned long j = 0; j < locations.size(); j++) {
		...
		if (locations[j].locations.size() > 0) {
			// layer_num.push_back(1);
			printLocations(locations[j].locations, layer + 1, detailed,
						   layer_num);
		}
		layer_num.back()++;
	}
...
int ConfigFileParser::testFunction(std::string const& key,
								   std::vector<std::string>& args,
								   int& lineCount) {
	int i;
	if (key == "server")
 ...
 */
int ConfigFileParser::SaveConfigData(std::vector<std::string>& args, int layer,
