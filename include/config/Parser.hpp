...
class ConfigFileParser {
	public:
		...
		int testFunction(std::string const& key, std::vector<std::string>& args,
						 int& lineCount);
	private:
		...
		int parseConfigFile(std::string& configFileName, int layer,
							unsigned long& i, int& lineCount);

		int handleServer(std::string& line, unsigned long* i);
		int handlePrompt(std::string& line, int layer, int& lineCount);
		int SaveConfigData(std::vector<std::string>& args, int layer,
						   int qoute_flag, int& lineCount);
		enum CmdId idCommand(std::string const& command);

		int server(std::vector<std::string>& args, int& lineCount, int layer);
		int location(std::vector<std::string>& args, int const& lineCount,
					 int layer);

		int listen(std::vector<std::string>& args, int& lineCount, int layer);
		int serverName(std::vector<std::string>& args, int& lineCount,
					   int layer);
		int errorPage(std::vector<std::string>& args, int& lineCount,
					  int layer);
		int clientMaxBodySize(std::vector<std::string>& args, int& lineCount,
							  int layer);
		int limitExcept(std::vector<std::string>& args, int& lineCount,
						int layer);
		int returnKeyword(std::vector<std::string>& args, int const& lineCount,
						  int layer);
		int root(std::vector<std::string>& args, int& lineCount, int layer);
		int autoindex(std::vector<std::string>& args, int& lineCount,
					  int layer);
